...
void histOpenfiles()
{
  ...
  printf(" Enter name of *.xls OUTPUT histogram file (example: lenahist.xls) ");
  scanf("%s", &outfile);
...
//----------------------------------------------------------------------------//
//         Obtain Histogram file and open in Excel to verify                  //
//----------------------------------------------------------------------------//

void histUserdefined()
{
  unsigned int histcount[256], Pixel;
  char string[10];
  ...
  // Read Pixels from image and increment histcount
  Pixel = fgetc(infptr);
  do
  {
  ...
  // Convert histcount entries from integer to string
  for (int i = 0; i < 256; i++)
  {
